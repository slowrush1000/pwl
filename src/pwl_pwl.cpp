...
void pwl::PWL::addXY(const pwl::XY &xy) { mXYs.emplace_back(xy); }

std::vector<pwl::XY> &pwl::PWL::getXYs() { return mXYs; }
...
pwl::PWL::getOverlapPeriod(const pwl::xy_tt x1, const pwl::xy_tt x2,
                           const pwl::xy_tt x3, const pwl::xy_tt x4,
                           bool &isOverlap)
{
    auto x31 = x3;
    auto x41 = x4;
...
bool pwl::PWL::isIn(const pwl::xy_tt x, const pwl::xy_tt x1,
                    const pwl::xy_tt x2)
{
    if ((x1 <= x) && (x <= x2))
    {
...
pwl::xy_tt pwl::PWL::getY(const pwl::xy_tt x)
{
...
pwl::xy_tt pwl::PWL::getAvg(const pwl::xy_tt x1, const pwl::xy_tt x2)
{
...
pwl::xy_tt pwl::PWL::getRMS(const pwl::xy_tt x1, const pwl::xy_tt x2)
{
    auto sum = pwl::kXYttInit;
    ...
    for (auto pos = 0; pos < (mXYs.size() - 1); ++pos)
    {
        ...
        {
            sum +=
                this->getAreaPow2(xy22.first, xy22.second, x11, y11, x12, y12);
        }
    }
...
pwl::xy_tt pwl::PWL::getMax(const pwl::xy_tt x1, const pwl::xy_tt x2)
{
    ...
    for (auto pos = 0; pos < (mXYs.size() - 1); ++pos)
    {
        ...
        if (true == isOverlap)
        {
            ...
            max =
                std::max<pwl::xy_tt>(max, std::max<pwl::xy_tt>(xy22y1, xy22y2));
        }
    }
...
pwl::xy_tt pwl::PWL::getMin(const pwl::xy_tt x1, const pwl::xy_tt x2)
{
    ...
    for (auto pos = 0; pos < (mXYs.size() - 1); ++pos)
    {
        ...
        if (true == isOverlap)
        {
            ...
            min =
                std::min<pwl::xy_tt>(min, std::min<pwl::xy_tt>(xy22y1, xy22y2));
        }
    }
...
pwl::xy_tt pwl::PWL::getMaxPeakToPeak(const pwl::xy_tt x1, const pwl::xy_tt x2)
{
...
pwl::xy_tt pwl::PWL::getArea(const pwl::xy_tt x1, const pwl::xy_tt x2,
                             const pwl::xy_tt x11, const pwl::xy_tt y11,
                             const pwl::xy_tt x12, const pwl::xy_tt y12)
{
...
pwl::xy_tt pwl::PWL::getAreaPow2(const pwl::xy_tt x1, const pwl::xy_tt x2,
                                 const pwl::xy_tt x11, const pwl::xy_tt y11,
                                 const pwl::xy_tt x12, const pwl::xy_tt y12)
{
    auto a = (y12 - y11) / (x12 - x11);
    ...
    return a * a * (x2 * x2 * x2 - x1 * x1 * x1) / 3.0 +
           a * b * (x2 * x2 - x1 * x1) + b * b * (x2 - x1);
}

void pwl::PWL::xshift(const pwl::xy_tt xshift)
{
...
void pwl::PWL::yscale(const pwl::xy_tt yscale)
{
    for (auto pos = 0; pos < mXYs.size(); ++pos)
    {
...
void pwl::PWL::add(pwl::PWL &other)
{
    auto xs1 = this->getXs();
...
std::vector<pwl::xy_tt> pwl::PWL::getXs()
{
...
std::vector<pwl::xy_tt> pwl::PWL::getYs()
{
...
std::vector<pwl::xy_tt> pwl::PWL::mergeXs(const std::vector<pwl::xy_tt> &xs1,
                                          const std::vector<pwl::xy_tt> &xs2)
{
    std::vector<pwl::xy_tt> xs;
...
pwl::PWL &pwl::PWL::operator=(const pwl::PWL &other)
{
    if (this == &other)
    {
        return *this;
...
void pwl::PWL::readFile(const std::string &fileName, const std::size_t xPos,
...
void pwl::PWL::writeFile(const std::string &fileName)
{
