...
namespace pwl
{
    class PWL
    {
      public:
        ...
        std::pair<xy_tt, xy_tt> getOverlapPeriod(const xy_tt x1, const xy_tt x2,
                                                 const xy_tt x3, const xy_tt x4,
                                                 bool &isOverlap);
        ...
        std::vector<xy_tt> mergeXs(const std::vector<xy_tt> &xs1,
                                   const std::vector<xy_tt> &xs2);

        xy_tt getArea(const xy_tt x1, const xy_tt x2, const xy_tt x11,
                      const xy_tt y11, const xy_tt x12, const xy_tt y12);
        xy_tt getAreaPow2(const xy_tt x1, const xy_tt x2, const xy_tt x11,
                          const xy_tt y11, const xy_tt x12, const xy_tt y12);

        PWL &operator=(const PWL &other);

        void readFile(const std::string &fileName, const std::size_t xPos,
