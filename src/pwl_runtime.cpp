...
double pwl::Runtime::getCPUTimeSecs() {
  return double(std::clock() - mCPUTime) / double(CLOCKS_PER_SEC);
}

double pwl::Runtime::getWallTimeSecs() {
  return std::chrono::duration<double>(
             std::chrono::high_resolution_clock::now() - mWallTime)
      .count();
}

std::size_t pwl::Runtime::getPeakRSSBytes() {
#if defined(_WIN32)
  /* Windows -------------------------------------------------- */
...
std::size_t pwl::Runtime::getCurrentRSSBytes() {
#if defined(_WIN32)
  /* Windows -------------------------------------------------- */
  PROCESS_MEMORY_COUNTERS info;
  GetProcessMemoryInfo(GetCurrentProcess(), &info, sizeof(info));
...
std::string pwl::Runtime::getRuntimeStr() {
  std::string runtimeStr = fmt::sprintf(
      "cpu %.1f secs, wall %.1f secs, peak %.1f MB, current %.1f MB",
      this->getCPUTimeSecs(), this->getWallTimeSecs(),
      this->getPeakRSSBytes() / 1.0e6, this->getCurrentRSSBytes() / 1.0e6);
